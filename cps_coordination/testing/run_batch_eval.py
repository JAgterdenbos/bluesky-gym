...
machinery ``run_cps_eval.py`` already exercises for a single (k_cps, mode)
run, adding:

  - a parameter sweep across ``k_cps`` and ``runway_assignment_mode``
    (default: 0/1/3 x static/dynamic, 6 combinations), reusing one shared
...
def _parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(
        description=(
            ...
            "runway_assignment_mode, two-pass (CPS + solo) per episode, "
            "streaming telemetry to Parquet. Production driver for the "
            "M=10,000 scale-up (Phase III roadmap Step 10)."
        ),
    ...
    p.add_argument("--seed-base", type=int, default=0,
                   help="Episode seed = seed_base + episode_index; matched between "
                        "the CPS and solo passes of the same episode.")
